...
router.get('/discover', async (req, res, next) => {
  try {
    const out = await discovery.discoverTeams(pool, {
      userId: req.user.id, sport: req.query.sport, q: req.query.q,
    });
 ...
 * Nothing here decides anything; it connects, unwraps and sends.
 */
router.get('/:id/suggested-players', async (req, res, next) => {
  const client = await pool.connect();
  try {
    const out = await roster.suggestPlayers(client, {
