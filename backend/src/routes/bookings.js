...
router.post('/', authMiddleware, checkRole('player'), async (req, res, next) => {
  ...
  try {
    ...
    // 1. Lock the slot row and verify it's available or locked BY THIS player
      ...
      `SELECT s.*, v.name as venue_name, v.price_per_hour, v.owner_id,
       ...
       WHERE s.id=$1 AND s.venue_id=$2 
         AND (s.status = 'available' 
              OR (s.status = 'temporarily_locked' AND s.locked_by = $3))
       FOR UPDATE`,
      [slotId, venueId, req.user.id]);
      
    if (!slotRes.rows.length) {
      await client.query('ROLLBACK');
      return res.status(409).json({ success: false, message: 'Slot no longer available. It may have been booked by someone else.' });
    }
    ...
    if (!playerWallet.rows.length || parseFloat(playerWallet.rows[0].balance) < amountToPay) {
      await client.query('ROLLBACK');
    ...
    // 5. Create booking
    const localSlotDateStr = slot.slot_date instanceof Date ? slot.slot_date.toLocaleDateString('en-CA') : slot.slot_date;
    const booking = await client.query(`
      ...
      RETURNING *`,
      [req.user.id, venueId, slotId, localSlotDateStr, slot.start_time,
       slot.end_time, basePrice, amountToPay, totalAmount, qrData, notes || null]);
       
    // 6. Mark slot as booked and clear lock metadata
    await client.query(
      `UPDATE slots SET status='booked', locked_at=null, locked_by=null WHERE id=$1`, [slotId]);
    
    // 7. Deduct from player available balance and ADD to player frozen balance (escrow)
    const newPlayerBalance = parseFloat(playerWallet.rows[0].balance) - amountToPay;
    const newPlayerFrozen = parseFloat(playerWallet.rows[0].frozen_balance || 0) + amountToPay;
    await client.query(
    ...
    await client.query(`
      ...
      [playerWallet.rows[0].id, req.user.id, booking.rows[0].id,
       -amountToPay, newPlayerBalance, 
       `Booking payment at ${slot.venue_name}`, slot.venue_name]);

    // 9. Release any other locks this player has on this venue
    await client.query(
      `UPDATE slots SET status='available', locked_at=null, locked_by=null
       WHERE venue_id=$1 AND locked_by=$2 AND status='temporarily_locked' AND id!=$3`,
      [venueId, req.user.id, slotId]);
...
router.patch('/:id/cancel', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (isLateCancel) {
      ...
      // 3. Transactions
      const playerWallet = await client.query(`SELECT id, balance FROM wallets WHERE user_id=$1`, [playerId]);
...
router.post('/:id/resolve', authMiddleware, checkRole('owner'), async (req, res, next) => {
  const client = await pool.connect();
  try {
    const { status } = req.body;
    if (!['completed', 'no_show'].includes(status)) {
      return res.status(400).json({ success: false, message: 'Invalid status. Must be completed or no_show' });
    }
    ...
    if (!['confirmed', 'checked_in'].includes(b.rows[0].status)) {
      await client.query('ROLLBACK');
      return res.status(400).json({ success: false, message: 'Booking is already resolved or cancelled' });
    }
    ...
    const playerWallet = await client.query(`SELECT id, balance FROM wallets WHERE user_id=$1`, [b.rows[0].player_id]);
    await client.query(`
      ...
      [playerWallet.rows[0].id, b.rows[0].player_id, req.params.id, deposit, playerWallet.rows[0].balance,
       `Escrow released to venue owner`, b.rows[0].venue_name]);
       
    // Transaction for owner (Escrow Received)
    await client.query(`
      INSERT INTO transactions (wallet_id, user_id, booking_id, type, amount, balance_after, description, counterparty_name)
      VALUES ($1,$2,$3,'escrow_received',$4,$5,$6,$7)`,
      [ownerWallet.rows[0].id, req.user.id, req.params.id, deposit, ownerWallet.rows[0].balance,
       `Received booking payment`, b.rows[0].player_name]);

    // Update booking status
    await client.query(`UPDATE bookings SET status=$1 WHERE id=$2`, [status, req.params.id]);

    await client.query('COMMIT');
    res.json({ success: true, message: `Booking marked as ${status} and funds transferred to your wallet.` });
  } catch(e) {
    await client.query('ROLLBACK');
